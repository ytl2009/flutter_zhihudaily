...
class HtmlWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    print("the StrippedHtml is ***************" + strippedHtml.toString());
...
//    return new Wrap(children: _widgets);
  ...
  Widget parseFromStr (String htmlStr) {
    ...
    print("the html.Node body is ******************" + body.toString());
  ...
  void _parseNode(html.Node node) {
     print('--- _parseNode');
     print(node.toString());
     print("the node type is *********" + node.nodeType.toString());
    ...
    // TODO fig caption
      ...
      // NOTE assuming img with width=height=11 as inline image icon
        ...
        if ( isInlineIcon ) {
          ...
          _tryCloseCurrentTextSpan();
          print("the enter the _parseElement method and the case is img *********" + element.className);
          if (element.className=="avatar"){
            return;
          }
      ...
      // if contains only one text node
        if ( element.hasContent() && ( element.nodes.length == 1 ) && ( element.firstChild.nodeType == html.Node.TEXT_NODE ) ) {
          ...
          _appendToCurrentTextSpans(_textLink(context: context, text: text, href: href));
  ...
  void _appendToCurrentTextSpans(dynamic stringOrTextSpan){
     print('=== appending to _currentTextSpan: ' + stringOrTextSpan.toString());
